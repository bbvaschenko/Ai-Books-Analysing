...
# Импортируем автономный классификатор учебности
        ...
        for filename in os.listdir(self.tags_directory):
            if filename.endswith(".txt") and filename != "области_знаний.txt":
                ...
                try:
                    ...
                    print(f"Ошибка загрузки тегов из {filename}: {e}")
                    tags_dict[category] = []
    ...
    def load_area_mapping(self, mapping_file: str = "tags/области_знаний.txt") -> Dict[str, str]:
        ...
        if os.path.exists(mapping_file):
            try:
                ...
                print(f"Ошибка загрузки соответствий: {e}")
        else:
...
class BookAnalyzer:
    ...
    def __init__(self, excel_file: str = "analyzed_books.xlsx"):
        # Используем абсолютный путь к файлу Excel
        ...
        # Инициализация автономного классификатора учебности
        print("Инициализация автономного классификатора учебной литературы...")
        self.educational_classifier = AutonomousEducationalClassifier()
        print("✅ Классификатор учебности готов к работе")
    ...
    def _ensure_excel_file(self):
        ...
        if not os.path.exists(self.excel_file):
            ...
            # Создаем базовую структуру DataFrame
            df = pd.DataFrame(columns=[
                "Номер книги", "ID книги", "Имя файла", "Область знаний", "Текст (фрагмент)",
                "Разделы", "Предметы", "Классы", "Авторы", "Темы",
                "AI_Резюме", "AI_Разделы", "AI_Ключевые_темы", "AI_Уровень_сложности",
                "AI_Предварительные_знания", "AI_Области_математики", "AI_Рекомендации"
            ])
    ...
    def get_next_book_number(self) -> int:
        ...
        try:
            ...
            # Находим максимальный номер книги
    ...
    def find_tags_for_text(self, text_embedding: torch.Tensor,
        ...
        try:
            ...
            print(f"⚠️ Ошибка при поиске тегов: {e}")
            return []
    ...
    def analyze_book(self, pdf_path: str) -> Optional[BookData]:
        ...
        if not check_result['is_educational']:
            ...
            print(f"   Файл отклонен.")
            return None

        print(f"\n✅ Проверка пройдена успешно!")

        # Генерация эмбеддингов
        print("\n🧠 Генерация эмбеддингов по тексту...")
        try:
        ...
        print(f"   🧭 Область знаний: {found_area[0]}")

        # AI анализ содержания книги
        print("\n🤖 Запуск расширенного AI анализа...")
        ai_analysis = None
        try:
            ...
            if ai_analysis.key_topics:
                print(f"🏷️  Ключевые темы: {', '.join(ai_analysis.key_topics[:5])}")
            if ai_analysis.recommendations:
                print(f"💡 Рекомендации: {ai_analysis.recommendations[0]}")
            print("="*60)

        except Exception as e:
            print(f"⚠️  Ошибка при AI анализе: {str(e)}")
        ...
        print(f"\n📚 Создаю запись о книге:")
        print(f"   Номер книги: {book_number}")
    ...
    def save_to_database(self, book_data: BookData):
        ...
        if book_data.ai_analysis:
            ...
            if ai_dict['sections_summary']:
                ...
                book_dict["AI_Разделы"] = ""

            book_dict["AI_Ключевые_темы"] = ', '.join(ai_dict['key_topics'][:5]) if ai_dict['key_topics'] else ""
            book_dict["AI_Уровень_сложности"] = ai_dict['difficulty_level']
            book_dict["AI_Предварительные_знания"] = ', '.join(ai_dict['prerequisites'][:3]) if ai_dict['prerequisites'] else ""
            book_dict["AI_Области_математики"] = ', '.join(ai_dict['mathematical_areas']) if ai_dict['mathematical_areas'] else ""
            book_dict["AI_Рекомендации"] = ' | '.join(ai_dict['recommendations'][:3]) if ai_dict['recommendations'] else ""
        ...
        try:
            if os.path.exists(self.excel_file):
                ...
                # Проверяем, нет ли уже книги с таким ID
                if 'ID книги' in existing_df.columns:
                    if book_data.book_id in existing_df['ID книги'].values:
                        print(f"⚠️  Книга с ID {book_data.book_id} уже существует в базе")
            ...
            # Сохраняем в Excel
            df.to_excel(self.excel_file, index=False)
            print(f"✅ Данные успешно сохранены в файл: {self.excel_file}")
